...
- **15 秒建议拆 5 秒×3 拼接**:三段 5 秒(564 秒)比一段 15 秒(898 秒)快且易控
...
| 配置 | 耗时 |
|---|---|
| T2I 22帧·8步 1024² | 87 秒 |
| T2I 5帧·8步 | 47 秒 |
| T2I 22帧·20步 | 314 秒(≈5 分钟) |
| I2I 22帧·8步 | ~90 秒 |
...
- **视频参考在 8GB 上不可行**:源视频(5 秒)+ Ref2VA 条件编码 >20 分钟且吃满显存 → 放弃
- **图片参考是正解**:单张参考图 189 秒出 5 秒视频,身份/服装保持可用
...
8GB 上 H3 生图太慢(约 90 秒),面板已内置专职图像模型后端:

| 模型 | 参数 | 耗时 | VRAM | 特点 |
|---|---|---|---|---|
| NoobAI-XL 1.0(SDXL) | 20步·cfg5·euler_ancestral/karras | 14-18 秒 | 7.2-7.4GB | 风格自由,中文/英文均可 |
| Krea-2-Turbo NVFP4 | 8步·cfg1·er_sde/simple | 26 秒 | 7.4GB | 字面/构图遵循强,Blackwell 原生 |
